...
import (
	...
	"github.com/s-amano/unknown-diary/backend/comconfirm-lambda/functions/diary-update/controller"
...
func handler(ctx context.Context, apiGWEvent events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	// POSTしたユーザー名取得
	getter = apiGWEvent.RequestContext.Authorizer["claims"].(map[string]interface{})["cognito:username"].(string)

	fmt.Printf("getter: %v\n", getter)

	id = apiGWEvent.QueryStringParameters["id"]

	// コントローラの作成
	src := controller.UpdateController{
